...
def run_export(robot_name, output_folder, mesh_quality="Medium"):
    ...
    try:
        _log(
            f"Iniciando exportación → robot='{robot_name}', "
            f"carpeta='{output_folder}', quality='{mesh_quality}'"
        )
        # Pasamos la calidad al exportador (robot.export_robot debe aceptar mesh_quality)
        robot.export_robot(robot_name, output_folder, mesh_quality=mesh_quality)
...
def on_execute(args):
    ...
    try:
        ...
        try:
            ...
            if q_input and q_input.selectedItem:
                mesh_quality = q_input.selectedItem.name
        except:
            # Si algo falla, seguimos con el valor por defecto
            pass

        _log(
            f"Ejecutando on_execute(): robot={robot_name}, "
            f"folder={output_folder}, quality={mesh_quality}"
        )
        run_export(robot_name, output_folder, mesh_quality)
...
def build_ui(cmd: adsk.core.Command):
    ...
    try:
        ...
        # Selector de calidad de malla para los .dae
        quality_input = inputs.addDropDownCommandInput(
            "meshQuality",
            "Mesh quality",
